...
public class ArrivalsParser {
    ...
    public static void parseArrivals(Stop stop, String jsonResponse)
        ...
        JSONArray received = new JSONArray(jsonResponse);

        for (int index = 0; index < received.length(); index++) {
            JSONObject element = received.getJSONObject(index);
            if (element.has("RouteNo")) {
                Route r = RouteManager.getInstance().getRouteWithNumber(element.getString("RouteNo"));
                stop.addRoute(r);
                if (element.has("Schedules")) {
                    parseArrival(element.getJSONArray("Schedules"), r, stop);
                }
            }
        }
    ...
    private static void parseArrival(JSONArray arrivals, Route r, Stop stop) throws JSONException, ArrivalsDataMissingException {
        ...
        for (int index = 0; index < arrivals.length(); index++) {
            ...
            if (arrival.has("ExpectedCountdown") && arrival.has("Destination") && arrival.has("ScheduleStatus")) {
                expectedCountdown = arrival.getInt("ExpectedCountdown");
                destination = arrival.getString("Destination");
                scheduleStatus = arrival.getString("ScheduleStatus");

                Arrival newArrival = new Arrival(expectedCountdown, destination, r);
